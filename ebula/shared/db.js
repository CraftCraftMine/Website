/**
 * Project:     CCMC™ Web EBuLa
 * Part of:     CraftCraftMine Netzwerk™ (CCMN™)
 * Copyright:   (c) 2026 CraftCraftMine Netzwerk™ – All rights reserved.
 */

/* ═══════════════════════════════════════
   shared/db.js — localStorage + Migration
═══════════════════════════════════════ */

const DB = {
  KEY: 'ccmn_ebula_v5',

  _migrObj(o) {
    if (o.gradient  === undefined) o.gradient  = null;
    if (o.speed     === undefined) o.speed     = null;
    if (o.speedEnd  === undefined) o.speedEnd  = null;
    if (o.notes     === undefined) o.notes     = '';
    if (!o.id) o.id = String(Date.now() + Math.random());
    if (o.type === 'STATION') {
      if (!Array.isArray(o.gleise)) o.gleise = [];
      o.gleise.forEach(g => {
        if (!g.id)           g.id   = String(Date.now() + Math.random());
        if (!g.name)         g.name = '';
        if (g.km === undefined) g.km = null;
      });
    }
    return o;
  },

  migrate(r) {
    if (!r) return null;
    if (!r.version)     r.version    = 1;
    if (!r.timetables)  r.timetables = [];
    if (!r.objects)     r.objects    = [];
    if (!r.createdAt)   r.createdAt  = new Date().toISOString();
    if (!r.desc)        r.desc       = '';
    if (!r.author)      r.author     = '';
    if (!r.streckenNr)  r.streckenNr = '';
    if (!r.zugTyp)      r.zugTyp     = '';
    if (r.objects_rueck === undefined) r.objects_rueck = null;

    r.objects.forEach(o => this._migrObj(o));
    if (r.objects_rueck) r.objects_rueck.forEach(o => this._migrObj(o));

    r.timetables.forEach(tt => {
      if (!tt.stops)             tt.stops     = [];
      if (!tt.mode)              tt.mode      = 'TIME';
      if (!tt.departure)         tt.departure = '';
      if (!tt.avgSpeed)          tt.avgSpeed  = 100;
      if (!tt.createdAt)         tt.createdAt = new Date().toISOString();
      if (!tt.zugNummer)         tt.zugNummer = '';
      if (!tt.zugTyp)            tt.zugTyp    = '';
      if (!tt.id)                tt.id        = String(Date.now() + Math.random());
      if (tt.richtung === undefined) tt.richtung = 1;
      if (!tt.direction) tt.direction = 'hin';
      // IBIS-artige Fahrplan-Nummer (kurze Zahl, z.B. "9999")
      if (tt.fpNr === undefined) tt.fpNr = '';
      tt.departure = this._normTime(tt.departure);
      tt.stops.forEach(s => {
        if (s.served  === undefined) s.served = true;
        if (!s.arr)   s.arr = '';
        if (!s.dep)   s.dep = '';
        s.arr = this._normTime(s.arr);
        s.dep = this._normTime(s.dep);
        if (s.gleis === undefined) s.gleis = null;
      });
    });

    r.version = EBULA_VERSION;
    return r;
  },

  get() {
    try {
      const raw = localStorage.getItem(this.KEY) || '[]';
      return JSON.parse(raw).map(r => this.migrate(r)).filter(Boolean);
    } catch { return []; }
  },

  set(routes) { localStorage.setItem(this.KEY, JSON.stringify(routes)); },

  save(route) {
    const all = this.get();
    const idx = all.findIndex(r => r.id === route.id);
    if (idx >= 0) all[idx] = route; else all.push(route);
    this.set(all);
  },

  getById(id)   { return this.get().find(r => r.id === id) || null; },

  getByName(name) {
    const n = name.toLowerCase().trim();
    return this.get().find(r =>
      r.name.toLowerCase().replace(/\s+/g,'-') === n ||
      r.name.toLowerCase() === n
    ) || null;
  },

  delete(id) { this.set(this.get().filter(r => r.id !== id)); },

  saveTimetable(routeId, tt) {
    const route = this.getById(routeId);
    if (!route) return;
    const tts = route.timetables || [];
    const idx = tts.findIndex(t => t.id === tt.id);
    if (idx >= 0) tts[idx] = tt; else tts.push(tt);
    this.save({ ...route, timetables: tts });
  },

  deleteTimetable(routeId, ttId) {
    const route = this.getById(routeId);
    if (!route) return;
    this.save({ ...route, timetables: (route.timetables||[]).filter(t => t.id !== ttId) });
  },

  initRueck(routeId) {
    const route = this.getById(routeId);
    if (!route || route.objects_rueck) return route;
    const rueck = (route.objects||[]).map(o => ({
      ...JSON.parse(JSON.stringify(o)),
      id: String(Date.now() + Math.random()),
      gleise: (o.gleise||[]).map(g => ({...g, id: String(Date.now() + Math.random())})),
    }));
    const updated = { ...route, objects_rueck: rueck };
    this.save(updated);
    return updated;
  },

  getActiveTTId(routeId) {
    try { return JSON.parse(localStorage.getItem('ccmn_active_tt')||'{}')[routeId] || null; }
    catch { return null; }
  },
  setActiveTTId(routeId, ttId) {
    try {
      const m = JSON.parse(localStorage.getItem('ccmn_active_tt')||'{}');
      m[routeId] = ttId;
      localStorage.setItem('ccmn_active_tt', JSON.stringify(m));
    } catch {}
  },

  _normTime(t) {
    if (!t || typeof t !== 'string') return t || '';
    const clean = t.trim();
    if (!clean) return '';
    const parts = clean.split(':').map(s => parseInt(s, 10));
    if (parts.some(isNaN)) return clean;
    const h = String(parts[0]||0).padStart(2,'0');
    const m = String(parts[1]||0).padStart(2,'0');
    const s = String(parts[2]||0).padStart(2,'0');
    return `${h}:${m}:${s}`;
  },

  tutDone:  () => localStorage.getItem('ccmn_tut6') === '1',
  markTut:  () => localStorage.setItem('ccmn_tut6', '1'),
  resetTut: () => localStorage.removeItem('ccmn_tut6'),

  getSettings() {
    try { return JSON.parse(localStorage.getItem('ccmn_settings') || '{}'); }
    catch { return {}; }
  },
  saveSettings(s) { localStorage.setItem('ccmn_settings', JSON.stringify(s)); },

  // Speicherpunkt: aktuelle Fahrtsituation sichern/laden
  savePunkt(data) {
    try { localStorage.setItem('ccmn_punkt', JSON.stringify(data)); } catch {}
  },
  loadPunkt() {
    try { const r = localStorage.getItem('ccmn_punkt'); return r ? JSON.parse(r) : null; }
    catch { return null; }
  },
  clearPunkt() { localStorage.removeItem('ccmn_punkt'); },
};

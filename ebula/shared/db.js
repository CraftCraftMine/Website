/* ═══════════════════════════════════════
   CCMN™ Web EBuLa — shared/db.js
   localStorage + Schema-Migration
   Erfordert: types.js vorher geladen
═══════════════════════════════════════ */
  ...
  migrate(r) {
    ...
    r.timetables.forEach(tt => {
      ...
      tt.stops.forEach(s => {
        ...
        if (!s.arr)   s.arr = '';
        if (!s.dep)   s.dep = '';
        // Zeiten: HH:MM → HH:MM:00
        if (s.arr && s.arr.split(':').length === 2) s.arr += ':00';
        if (s.dep && s.dep.split(':').length === 2) s.dep += ':00';
      });
    });
